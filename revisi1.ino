...
void updateTime() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if (now - lastUpdate >= 60000) {  // update time from NTP server every minute
      lastUpdate = now;
      timeClient.update();
    ...
    if (now - lastUpdate >= 1000) {  // update local time every second
      lastUpdate = now;
      setTime(hour(), minute(), second() + 1);  // Update only hours, minutes, and seconds
    }
  }
}
...
void printCurrentTime() {
  ...
  if (now - lastPrint >= 1000) {  // print time every second
    lastPrint = now;
...
//input nilai presentase & waktu sunset
BLYNK_WRITE(V4) {
  printTime(2, param);
}
BLYNK_WRITE(V12) {
...
//PROSES MERUBAH NILAI PWM 
  if (currentHour >= sunrise_Hour && currentMinute >= sunrise_Minute && sunrise_Hour > daylight_Hour && sunrise_Minute > daylight_Minute) {
    sigmoidFade(pwmValue_1, Pwm_A1, pwmValue_2, Pwm_B1, pwmValue_3, Pwm_C1); 
    ...
    Pwm_B4 = pwmValue_2;
    Pwm_C4 = pwmValue_3;
    analogWrite(PWM_PIN_1, (float)pwmValue_1);
    analogWrite(PWM_PIN_2, (float)pwmValue_2);
    analogWrite(PWM_PIN_3, (float)pwmValue_3);
...
void sigmoidFade(float startA, float endA, float startB, float endB, float startC, float endC) {
  ...
  for (int i = 0; i <= steps; i++) {
    ...
    analogWrite(PWM_PIN_1, (float)fadeA);
    analogWrite(PWM_PIN_2, (float)fadeB);
    analogWrite(PWM_PIN_3, (float)fadeC);
    Serial.println("fade a: " + String(fadeA));
    Serial.println("fade b: " + String(fadeB));
    Serial.println("fade c: " + String(fadeC));
